...
void TrajectoryAnalyzer::ToTrajectoryFrame(const double x, const double y,
  ...
  // (dx, dy)
  double cross_rd_nd = (dy) * (sin_ref_theta) + (dx) * (cos_ref_theta);//横向偏差x轴
  *ptr_d = cross_rd_nd;

  // the cos of diff angle between vector (cos_ref_theta, sin_ref_theta) and
  // (dx, dy)
  double dot_rd_nd = (dy) * (cos_ref_theta) - (dx) * (sin_ref_theta);// 纵向偏差y轴double dot_rd_nd = dx(0910-question) * (0910-question) + (0910-question) * (0910-question);
